...
void TProjection::filter_items_from_sequence(TSequence *tseq, int item)
{
	...
	bool del_elem;
	for( ; telem!=(tseq)->end(); elem_idx++, del_elem?(tseq)->erase(telem):telem++)
	{
		del_elem=0;
		...
		for( ; item_pt!=(*telem)->end(); item_pos++, del_item?(*telem)->erase(item_pt):item_pt++)
		{
			del_item=0;
										...
										projection_type type)
{
	...
	switch (type) {
		case EXTEND:
			...
			break;
		case ENLARGE:
			// -Method for type==ENLARGE: keep only annotations pointing to Elements where new_item occurs
			new_seq->annotations=checked_new(vector<EntryPoint>);
			for(entry=(*tseq)->annotations->begin(); entry != (*tseq)->annotations->end(); entry++)
				if(entry->entrypoint.idx >= orig_item_pointer.idx) 
				// No need to control elements that will be eliminated -- and entrypoints==(-1,0) are correctly skipped
				{
					TSequence::iterator scan_elem = (*tseq)->begin()+entry->entrypoint.idx;
					if(entry->entrypoint.idx==orig_item_pointer.idx && item_pointer.idx>orig_item_pointer.idx) 
						// The ENLARGED element (==1st occurrence) will be deleted => simply set entrypoint to (-1,0)
					{
						new_seq->annotations->push_back(*entry);	// Copy old annotation (times are left unchanged)
						new_seq->annotations->back().entrypoint=TItemPointer();
					} else 
						if((item=find((*scan_elem)->begin(), (*scan_elem)->end(), new_item)) != (*scan_elem)->end())
						// new_item found in position "item" (an iterator) => copy annotation, add item and update offset
						{
							new_seq->annotations->push_back(*entry);	// Copy old annotation (times are left unchanged)
							// Update entrypoint: offset:=item, idx left unchanged  
							new_seq->annotations->back().entrypoint.position=static_cast<int>(item-(*scan_elem)->begin());
						};
				};
			break;
		default:
			launch_error("Wrong extension/enlargement type in tseq. proj. generation!!");
	};
...
vector<TProjection*> *TProjection::generate_projections(bool disk_in, bool disk_out, float max_time_gap)
{
	...
	// Sequence::iterator last_prefix_el=((prefix->size()==0)?(prefix->begin()):(prefix->end()-1));
	//****************************************************************************************************//
	Sequence::iterator last_prefix_el/*=((prefix->size()==0)?((Sequence::iterator)0):(prefix->end()-1))*/;
	//****************************************************************************************************//
	int last_prefix_el_test;  // =0 <-> empty prefix 
	if ((prefix->size()==0)) {
		last_prefix_el_test = 0;
	}
	else {
		last_prefix_el_test = 1;
	...
	for( ; N_to_retrieve>0; N_to_retrieve-- ) // Foreach seq.
	{
		...
		if(!(*tseq)->empty())  // Empty sequences do not contribute with new postfixes => ignore them
		{
			...
			int enlarge_elem_idx=0;
			int extend_elem_idx=0;	// 
			unsigned int enlarge_item=0;
			local_extend.clear();
			local_enlarge.clear();
			...
			// General ENLARGEMENT
			if(last_prefix_el_test !=0  &&  (*tseq)->annotations!=0) // if no prefix at all => no ENLARGE
			{
				int prev_idx=-1;
			...
			// EXTENSION
			// TODO: max_time_gap is not FULLY activated, yet!   [  && ((*extend_elem)->time-XXX)<=max_time_gap  ]
			int lookahead=0;
			// Negative max_time_gap & not empty prefix => contiguous extension
			int max_tsequence_lookahead = ((max_time_gap<=0) && (last_prefix_el_test!=0)) ? 1 : 999999;
			for(TSequence::iterator extend_elem = (*tseq)->begin()+extend_elem_idx; // CHECK: extend_elem_idx==0 always, at this point
					extend_elem!=(*tseq)->end() && lookahead<max_tsequence_lookahead; 
					extend_elem++, extend_elem_idx++, lookahead++)
...
void TProjection::filter_annotations(H_store *O, float tau)
{
	...
	for(TProjection::iterator tseq=this->begin(); tseq!=this->end(); ) // tseq is incremented manually below...
	{
		...
		for(vector<EntryPoint>::iterator ann=my_annotations->begin(); ann!=my_annotations->end(); ann++)
		{
			...
			{
				tmp_h.low.push_back(*tt-base-tau);
...
H_store *TProjection::extract_dense_rectangles(float tau, indexing_strategy index_strat)
{
	...
	H_store *local_store = checked_new(H_store(n,index_strat));
	...
	// std::ofstream out_stream; //추출한 문자열 저장할 텍스트 파일	
	// out_stream.open("MiSTA.output",std::ios_base::out | std::ios_base::app);
	// std::cout <<"pPath : "<< " halo "<<endl;
	...
	{	
		...
		for(vector<EntryPoint>::iterator entry=(*seq)->annotations->begin(); entry!=(*seq)->annotations->end(); entry++)
		{
			...
			{
				tmp_h.low.push_back(APPROX_FLOAT(*tt-base-tau));
	...
	////DEBUG
	// std::cout << "Time intervals0:\n";
	// for(int i=0; i<tmp_vector.size(); ++i)
 //  		std::cout << tmp_vector[i].low << ' ';

	// std::cout << "Time intervals1:\n";
	// for (std::vector<H>::const_iterator abab = tmp_vector.begin(); abab != tmp_vector.end(); ++abab)
	// 	std::cout << *abab << ' ';  //std::cout


	// std::cout << "Time intervals2:\n";
	// for (std::vector<H>::const_iterator abab = annot_vector.begin(); abab != annot_vector.end(); ++abab)
	// 	std::cout << *abab << ' ';  //std::cout
	////END DEBUG
	...
	H_store *working = checked_new(H_store(n,index_strat));
	H_store *local_working = checked_new(H_store(n,index_strat));
...
/// Extract a set of rectangles representing frequent annotations
H_store *TProjection::extract_dense_rectangles_brute_force(float tau, indexing_strategy index_strat)
{
	if(this->prefix==0 || this->prefix->size()<2) // Sequences smaller than 2 elems cannot have transition times
		launch_error("Cannot extract frequent annotations on sequences with less than 2 elements!!");
...
/// Extract a set of rectangles representing frequent annotations
H_store *TProjection::extract_dense_rectangles_brute_force2(float tau, indexing_strategy index_strat)
{
	if(this->prefix==0 || this->prefix->size()<2) // Sequences smaller than 2 elems cannot have transition times
		launch_error("Cannot extract frequent annotations on sequences with less than 2 elements!!");
...
template<class T> void _myread(T& x) { TProjection_dump_file->read((char *)&x,sizeof(T)); };
void TProjection::retrieve_tseq_from_disk(TSequence* tseq, int prefix_len)
{
	...
	for(vector<EntryPoint>::iterator ann=tseq->annotations->begin(); ann!=tseq->annotations->end(); ann++)
	{
		...
		for(vector<float>::iterator tt=ann->times.begin(); tt!=ann->times.end(); tt++)
			_myread(*tt);
		_myread(ann->entrypoint.idx);
		_myread(ann->entrypoint.position);
	};
